...
conn_s <- dbConnect(RSQLite::SQLite(), dbname = ":memory:")
conn_d <- dbConnect(duckdb::duckdb(), dbdir = ":memory:")
...
DuckDb supports all of the standard data types in SQL (including extended types):
`BOOLEAN`, `TINYINT`, `SMALLINT`, `INTEGER`, `BIGINT`, `FLOAT`, `DOUBLE`, `VARCHAR`, `DATE`, `TIME`, `TIMESTAMP`, `INTERVAL`, `HUGEINT`.

DuckDb also supports some additional types, most notably the nested data types: `LIST`, `STRUCT`, `MAP` and `UNION`.
